<template>
  <div>
    <el-row class="header">
      <el-col :span="18"><h4>医院科室信息</h4>
      </el-col>
      <el-col :span="6">
        <el-button type="primary" v-if="canIVisit('operate', 'ORGANIZATION_DEPARTMENT_CREATE')" plain icon="el-icon-plus" @click="goToDetail('add', '')">添加科室信息</el-button>
      </el-col>
    </el-row>
    <el-table :data="departmentListInfo" style="width: 100%">
      <el-table-column type="expand"> v-if="canIVisit('operate', 'DEPARTMENT_READ')"
        <template slot-scope="props">
          <el-table :data="props.row.secondDepartment">
            ...
            <el-table-column align="left" label="操作" width="240" v-if="canIVisit('operate', 'ORGANIZATION_DEPARTMENT_READ') || canIVisit('operate', 'ORGANIZATION_DEPARTMENT_UPDATE') || canIVisit('operate', 'ORGANIZATION_DEPARTMENT_DELETE')">
              <template slot-scope="scope">
                <el-button size="mini" v-if="canIVisit('operate', 'ORGANIZATION_DEPARTMENT_READ')" type="success" plain @click="goToDetail('check',scope.row.id)">查看</el-button>
                <el-button size="mini" v-if="canIVisit('operate', 'ORGANIZATION_DEPARTMENT_UPDATE')" type="primary" plain @click="goToDetail('update', scope.row.id)">修改</el-button>
                <el-button size="mini" v-if="canIVisit('operate', 'ORGANIZATION_DEPARTMENT_DELETE')" type="danger" plain @click="deleteRow(scope.row)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
        </template>
...
export default {
  ...
  methods: {
    ...
    getInfo(page) {
      let url = `${conf.url}/internethospital/api/departments?page-index=${page}&page-size=10&type=HOSPITAL_DEPARTMENT&show-all=true&stand-department=false`;
