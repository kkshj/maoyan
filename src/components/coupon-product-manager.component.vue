...
export default {
  ...
  methods: {
    ...
    //初始化数据
    getInfo(page) {
      const url = `${conf.url}/internethospital/api/products?page-index=${page}&page-size=10&type=COUPON`;
    ...
    goToDetail(type, id, ) {
      this.$router.push({
        path: '/coupon-product-manager/detail',
        query: {
          type: type,
          id: id
        }
