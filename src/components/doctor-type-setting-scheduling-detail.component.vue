    ...
    <el-form ref="doctorInformation" :model="doctorInformation" label-width="160px" :rules="rules" style="width: 70%; margin: 20px auto;">
      ...
      <el-form-item label="接诊日期" prop="startAndEndTime">
        ...
        <div v-else class="block">
          <el-date-picker v-model="doctorInformation.startAndEndTime" type="datetimerange" align="right" start-placeholder="开始日期" end-placeholder="结束日期">
          </el-date-picker>
        </div>
      </el-form-item>
  ...
  methods: {
    ...
    getInfo(id) {
      if (this.type === 'add') {
        ...
        ax.get(url, this.header).then(res => {
          res.data.startAndEndTime = [new Date(res.data.startDateTime), new Date(res.data.endDateTime)];
          this.doctorInformation = res.data;
        }).catch(err => {
          this.refreshTokened(err)
          console.log(err);
    ...
    create() {
      this.$refs['doctorInformation'].validate((data) => {
        if (!data) {
          ...
          if (this.doctorInformation.startAndEndTime == undefined) {
            this.$message({
              type: 'warning',
              message: '请选择接诊日期'
            });
