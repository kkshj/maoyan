<template>
  <div>
    <el-row class="header">
      <el-col :span="12"><h4>产品信息{{type === 'add' ? '录入' : type === 'update' ? '编辑' : type=='check '? '查看':''}}</h4>
      </el-col>
      <el-col :span="12">
    ...
    <el-form ref="goods" :model="Product" label-position="left" label-width="100px" :rules="rules" style="width: 70%; margin: 20px auto;">
      ...
      <!--<el-form-item label="拼音缩写" prop="abbreviation">-->
      <!--<span v-if="type === 'check'">{{Product.abbreviation}}</span>-->
      <!--<el-input v-else v-model="Product.abbreviation"></el-input>-->
      <!--</el-form-item>-->
      ...
      <el-form-item label="单位" prop="standard">
        <span v-if="type === 'check'">{{Product.shippingUnitText}}</span>
        <el-input v-else v-model="Product.shippingUnitText  "></el-input>
      </el-form-item>
      <el-form-item label="规格" prop="standard">
        <span v-if="type === 'check'">{{Product.standard}}</span>
        <el-input v-else v-model="Product.standard"></el-input>
      </el-form-item>
      <el-form-item label="拼音首字母" prop="pinyinAbbreviate">
        <span v-if="type === 'check'">{{Product.pinyinAbbreviate}}</span>
        <el-input v-else v-model="Product.pinyinAbbreviate"></el-input>
      </el-form-item>
      <el-form-item label="价格" prop="price">
        <span v-if="type === 'check'">￥{{Product.price}}</span>
        <el-input v-else v-model="Product.price"></el-input>
      </el-form-item>
